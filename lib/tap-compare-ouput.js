// Thanks rvagg!
//
// https://github.com/rvagg/workshopper-exercise/blob/7301de02e2d62cceb6ced110b8c71e683c4c43ad/comparestdout.js
...
function processor (mode, callback) {
  ...
  this.submissionStdout.pipe(parser(function (submission) {
    this.solutionStdout.pipe(parser(function (solution) {
      outputStream.write('\nYour submission results compared to the expected:\n\n')

      if (!this.longCompareOutput)
        outputStream.write(chalk.yellow(center('ACTUAL', 40) + center('EXPECTED', 40) + '\n'))

      outputStream.write(chalk.yellow(repeat('\u2500', 80)) + '\n\n')

      Object.keys(solution).forEach(function (key, value) {
        console.log(solution[key]);
        var chunk = [ submission[key], solution[key] ];
